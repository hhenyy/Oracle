...
--Q2. 사원 테이블에서 사원명이 알파벳으로 가장 먼저 나오는 사원명을 구하는
-- SQL문 작성?
select min(ename) from emp; --ADAMS
select ename from emp order by ename asc; --오름차순정렬(사전순 정렬)
...
select count(mgr) from emp;  --13 : null값은 counting을 하지않는다.
select count(comm) from emp; --4 : null값은 counting을 하지않는다.
select count(deptno) from dept; --4 : deptno 컬럼은 기본키 제약조건이 설정되어 있다.
select count (empno) from emp; --14 : empno컬럼은 기본키 제약조건이 설정되어 있다.
select count (*) from emp; 

--Q1. 사원테이블에서 중복행을 제거한 JOB의 갯수를 구하는 SQL문 작성?
--1) job의 갯수 구하기?
...
--Q. 각 부서(10,20,30)의 급여합, 평균급여, 최대급여, 최소급여를 구하는 SQL문 작성?
select sum(sal),avg(sal),max(sal),min(sal) from emp where deptno=10;
select sum(sal),avg(sal),max(sal),min(sal) from emp where deptno=20;
select sum(sal),avg(sal),max(sal),min(sal) from emp where deptno=30;

-- 그룹함수와 일반칼럼은 같이 사용할 수 없지만, 예외적으로 group by절에 사용되는
...
--having 조건걸
--:group by 절이 사용되는 경우에 데이터 제한을 가하기 위해서 where 조건절 
-- 대신에 having 조건절을 사용해야 한다.

--Q1. 각 부서별(10,20,30) 평균급여 금액이 2000이상인 부서만 출력하는 SQL문 작성?
...
--Q2. 각 부서별(10,20,30) 최대급여 금액이 2900 이상인 부서만 출력하는 SQL문 작성?
--1) 각 부서별 최대급여 금액 출력
